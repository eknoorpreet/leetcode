...
// Go through all islands and calculate the area for each
...
const maxAreaOfIsland = function (grid) {
  ...
  const dfs = (r, c) => {
    ...
    // Base case: if the cell is out of bounds,
    // contains water,
    // or has already been visited (island exhausted) => don't add more to area (just add 0)
    if (
      r < 0 ||
      r === rows ||
      c < 0 ||
      ...
      return 0;
    // Otherwise, keep on exploring the island
    // Mark it visited
    visited.add(`${r}-${c}`);
    let localArea = 1;
...
Note 1: It's not necessary to check the above conditions since the DFS function already checks
them and returns 0 if they're not fulfiled.
But you can skip the DFS call entirely for cells that are not part of an island
...
In the worst case, it visits each cell of the grid, making it O(m * n),
where 'm' is the number of rows and 'n' is the number of columns in the grid.
Within the DFS, for each cell that belongs to an island, it may explore up to
four neighboring cells (up, down, left, right).
Therefore, the time complexity of the DFS part is O(4 * m * n), which simplifies to O(m * n).
Overall, the time complexity of the code is O(m * n) because the DFS traversal is the
dominant factor.
...
The recursive DFS calls also consume space on the call stack. In the worst case,
the maximum depth of the call stack is equal to the number of cells in the grid.
