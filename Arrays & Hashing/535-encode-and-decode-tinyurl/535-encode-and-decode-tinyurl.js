...
/*
 ...
 Considerations: How to decide the encoded url? Well, no restrictions => just count

 encodeMap["leetcode"] : 1
 decodeMap[1] : "leetcode"

 */

//TC: O(1)
const encode = function (longUrl) {
  //if the original URL has not been encoded before
  if (!encodeMap.has(longUrl)) {
    //encode it
    const shortUrl = `${base}${encodeMap.size + 1}`;
    //add it to the encodeMap
    encodeMap.set(longUrl, shortUrl);
    //add it to decodeMap (so we can decode it later)
