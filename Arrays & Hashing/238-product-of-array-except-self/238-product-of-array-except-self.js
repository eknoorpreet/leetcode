...
const productExceptSelf3 = (nums) => {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    for (let j = 0; j < nums.length; j++) {
      //except when it's the same element
      if (i === j) continue;
...
const productExceptSelf2 = (nums) => {
  const product = [];
  for (let i = 0; i < nums.length; i++) {
    //for each index, get its left product * get its right product
...
const productExceptSelf1 = function (nums) {
  ...
  for (let i = 1; i < nums.length; i++) {
    //since left[i] is product of all "left" uptil ith index (i.e. it includes nums[i - 1], not nums[i])
    //(every index i in left contains product of nums[0] * .... * nums[i-1])
    //left[i]            =  nums[0] * .... * nums[i - 2] * nums[i - 1]
    //left[i - 1]        =  nums[0] * .... * nums[i - 2]
    //basically, left[i] = left[i - 1] * nums[i - 1]
    left[i] = left[i - 1] * nums[i - 1];
  }
  for (let i = nums.length - 2; i >= 0; i--) {
    //since right[i] is product of all right after ith index (i.e. it includes nums[i + 1], not nums[i])
    ...
    //basically, right[i] = right[i + 1] * nums[i + 1]
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    /*For any i-th item in nums we should be able to calculate the product
      of everything but itself, by multiplying its left and right product! */
...
At each i, result[i] = productFromLeft * productFromRight. Since we can't have access to productFromLeft and productFromRight at the same time without storing them somewhere, we will set result[i] to productFromLeft when iterating from the start and we will multiply productFromRight to each result[i] as we iterate through from the end.
*/
...
const productExceptSelf = function (nums) {
  ...
  console.log(result);
  //(or just reuse prefix... but this is clearer!)
