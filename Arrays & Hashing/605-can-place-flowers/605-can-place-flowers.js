...
//Apprach 1: Copy the input array and extend it like: [0, ...arr, 0] to take care fo edge cases
//TC: O(n)
//SC: O(n)
...
//Apprach 2: take care of edge cases
//TC: O(n)
//SC: O(1)
const canPlaceFlowers = function (flowerbed, n) {
  //optimization
  ...
  for (let i = 0; i < flowerbed.length; i++) {
    ...
    //(since for the first spot, we should not have to check the prev one and for the last spot, we should have to check the next one)
    if (
      flowerbed[i] === 0 &&
      (i === 0 || flowerbed[i - 1] === 0) &&
